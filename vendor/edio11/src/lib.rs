        ...
        if let Some(inner) = &mut self.inner {
            if let Some(render_target) = &inner.render_target {
                ...
                let egui_output = self
                    .egui_ctx
                    .run(inner.input_handler.collect_input(dpi_ppp, effective_ppp), |ctx| {
...
pub fn set_overlay(
    ...
    set_overlay_with_zoom(overlay_creator, present_target, resize_buffers_target, 1.0)
}

pub fn set_overlay_with_zoom(
    overlay_creator: Box<dyn FnOnce(Context) -> Box<dyn Overlay>>,
    present_target: fn(*const IDXGISwapChain_Vtbl, u32, DXGI_PRESENT) -> HRESULT,
    resize_buffers_target: fn(
        *const IDXGISwapChain_Vtbl,
    ...
    match unsafe { &*overlay_handler } {
        ...
        None => unsafe {
            ...
            if let Err(e) = EnableMouseInPointer(InputHandler::pointer_input_enabled()) {
            ...
            let overlay_handler = OverlayHandler {
                ...
                zoom_factor: if initial_zoom_factor.is_finite() {
                    initial_zoom_factor.clamp(MIN_ZOOM_FACTOR, MAX_ZOOM_FACTOR)
                } else {
                    1.0
                },
            };
